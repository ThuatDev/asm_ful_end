...
const DetailQuiz = () => {
  ...
  const handleNext = () => {
    ...
    if (quizData && quizData.length > index + 1)
      //coi quizData.length nhu index cuoi cung cua array quizData[]
  ...
  const handleCheckBox = (answerId, questionId) => {
    ...
    if (question && question.answers) {
      ...
      question.answers = question.answers.map((answer) => {
        //map xong thi` gan' de update answers của question
        // console.log(answer);
        if (answer.id === answerId) {
          //trung id -> toggle checked
  ...
  return (
    <>
      <Breadcrumb className="quiz-detail-new-header">
        <NavLink to="/" className="breadcrum-item">
          {t("detailQuiz.title1")}
        </NavLink>
        <span> / </span>
        <NavLink to="/users" className="breadcrum-item">
          {t("detailQuiz.title2")}
        </NavLink>
        <span style={{ marginRight: "16px" }}> / </span>
        <Breadcrumb.Item active>{t("detailQuiz.title3")}</Breadcrumb.Item>
      </Breadcrumb>
